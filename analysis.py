...
class StockAnalysis:
    def __init__(
        ...
        contraction_max=50,
        contraction_min=15,
        weeks_of_contraction=2,
        local_high_low_order=10,
        min_contractions=2,
        max_contractions=4,
        vol_contraction_days=(5, 30),
    ):
        self.stock = stock
        ...
        self.period = period
        self.start_date = (datetime.today() - timedelta(days=1 * 365)).strftime(
            "%Y-%m-%d"
        )
        ...
        # Tunable parameters
        self.contraction_max = (
            contraction_max  # Maximum acceptable contraction percentage
        )
        self.contraction_min = (
            contraction_min  # Minimum acceptable contraction percentage
        )
        self.weeks_of_contraction = (
            weeks_of_contraction  # Minimum number of weeks of contraction
        )
        self.local_high_low_order = (
            local_high_low_order  # Sensitivity for local high/low detection
        )
        ...
        # (short-term, long-term) window for volume contraction
        self.vol_contraction_days = vol_contraction_days
    ...
    def ma_strategy(self):
        ...
        self.df["short_MA"] = self.df["Close"].rolling(
            int(self.short_MA)).mean()
        self.df["crosszero"] = np.where(
            self.df["short_MA"] < self.df["long_MA"], 1.0, 0.0
        )
    ...
    def _local_high_low(self):
        local_high = argrelextrema(
            self.df["High"].to_numpy(), np.greater, order=10)[0]
        local_low = argrelextrema(
            self.df["Low"].to_numpy(), np.less, order=10)[0]
    ...
    def _contractions(self, local_high, local_low):
        ...
        while i < len(local_low) and j < len(local_high):
            if local_low[i] > local_high[j]:
                contraction = round(
                    (self.df["High"][local_high[j]] -
                     self.df["Low"][local_low[i]])
                    / self.df["High"][local_high[j]]
                    * 100,
                    2,
                )
    ...
    def vcp_strategy(self):
        ...
        weeks_of_contraction = (
            len(self.df.index) - local_high[num_of_contraction - 1]
        ) // 5

        # Volume contraction
        self.df["long_avg_volume"] = (
            self.df["Volume"].rolling(
                window=self.vol_contraction_days[1]).mean()
        )
        self.df["short_avg_volume"] = (
            self.df["Volume"].rolling(
                window=self.vol_contraction_days[0]).mean()
        )
        vol_contraction = (
            self.df["short_avg_volume"].iloc[-1] < self.df["long_avg_volume"].iloc[-1]
        )

        # Checking conditions
        if (
            max_contraction > self.contraction_max
            or min_contraction > self.contraction_min
        ):
        ...
        self.vcp_patterns = [
            {
                "high": self.df["High"][local_high[i]],
                "low": self.df["Low"][local_low[i]],
                "date_high": self.df["Date"][local_high[i]],
                "date_low": self.df["Date"][local_low[i]],
            }
            for i in range(num_of_contraction)
        ]
    ...
    def buy_sell_signals(self):
        ...
        for i, row in self.df.iterrows():
            ...
            if self.df.loc[i, "position"] == -1:
                ...
                print(
                    "{:^7}{}{:^15}{}{:^15}".format(
                        i, buydate, buy_price, selldate, sell_price
                    )
                )
        print("")
        print("")
    ...
    def backtest(self):
        ...
        print("-" * 100)
        print(
            "{:^7}{:^15}{:^10}{:^15}{:^20}{:^20}{:^10}{:^20}{:^20}{:^20}{:^20}".format(
                "Sr. No",
                "Buy Date",
                "Buy Price($)",
                "Sell Date",
        ...
        for i, row in self.df.iterrows():
            if position == 0 and self.df.loc[i, "position"] == 1:
                ...
                total_sell_p, profitloss, total_profit, MA_wealth = self._sell_action(
                    balance, qty, total_buy_p, sell_p, total_profit, MA_wealth
                )
    ...
    def graph(self):
        ...
        ax.plot(
            ...
            self.df["MA_wealth"],
            color="black",
            label="MA strategy wealth",
        )
        ax.plot(
            self.df["Date"],
            self.df["LT_wealth"],
            color="red",
            label="Buy and hold wealth",
        )
    ...
    def render_plotly_plot(self):
        # Create a Plotly figure using stock data (with moving averages, buy/sell signals, and VCP patterns)
        fig = go.Figure()

        # Add the stock's Close price line
        fig.add_trace(go.Scatter(
            x=self.df['Date'], y=self.df['Close'], mode='lines', name='Close Price'))

        # Add the long and short moving averages
        fig.add_trace(go.Scatter(
            x=self.df['Date'], y=self.df['long_MA'], mode='lines', name=f'{self.long_MA}-day MA'))
        fig.add_trace(go.Scatter(
            x=self.df['Date'], y=self.df['short_MA'], mode='lines', name=f'{self.short_MA}-day MA'))

        # Plot buy/sell signals
        fig.add_trace(go.Scatter(x=self.df['Date'], y=self.df['buy'], mode='markers', marker=dict(
            color='green', symbol='triangle-up', size=10), name='Buy'))
        fig.add_trace(go.Scatter(x=self.df['Date'], y=self.df['sell'], mode='markers', marker=dict(
            color='red', symbol='triangle-down', size=10), name='Sell'))

        # Plot VCP highs and lows (only one legend entry for each)
        if self.vcp_patterns:
            # Extract the unique dates and prices for VCP highs and lows
            vcp_high_dates = [pattern['date_high']
                              for pattern in self.vcp_patterns]
            vcp_high_values = [pattern['high']
                               for pattern in self.vcp_patterns]
            vcp_low_dates = [pattern['date_low']
                             for pattern in self.vcp_patterns]
            vcp_low_values = [pattern['low'] for pattern in self.vcp_patterns]

            # Add VCP highs
            fig.add_trace(go.Scatter(x=vcp_high_dates, y=vcp_high_values, mode='markers', marker=dict(
                color='blue', symbol='circle', size=10), name='VCP High'))

            # Add VCP lows
            fig.add_trace(go.Scatter(x=vcp_low_dates, y=vcp_low_values, mode='markers', marker=dict(
                color='purple', symbol='x', size=10), name='VCP Low'))

        # Customize the layout
        fig.update_layout(
            title=f'{self.stock} Price Chart with Buy/Sell Signals and VCP Patterns',
            xaxis_title='Date',
            yaxis_title='Price',
            showlegend=True
        )
...
if __name__ == "__main__":
    analysis = StockAnalysis(stock="MSFT")
    ...
    analysis.graph()
    html_plot = analysis.render_plotly_plot()  # Get the graph as HTML

    # You can now serve this HTML in a web app, return it from an Azure Function, or save it as a file
